...
it.layer(NodeServices.layer)("discoverClaudeSkills", (it) => {
  ...
  it.effect("discovers shared project and installed plugin skills from an active worktree", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-claude-skills-" });
      ...
      yield* writeSkill(
        ...
        "review",
        ["---", "name: review", "description: Shared project review.", "---"].join("\n"),
      );
      yield* writeSkill(
        path.join(packageDirectory, ".claude", "skills"),
        "hidden",
      ...
      yield* writeSkill(
        ...
        "triage",
        ["---", "name: triage", "description: Triage a bug.", "---"].join("\n"),
      );

      const skills = yield* discoverClaudeSkills({ homePath: configDir }, packageDirectory);
      ...
      assert.deepEqual(
        ...
        [
          ...
          { name: "review", scope: "project", description: "Shared project review." },
        ],
      );
    }),
  );
